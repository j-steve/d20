 ...
 * 
 * @class
 * @param {String} arg1     the name of the database table asociated with this class, or the schema name if two arguments are passed.
 * @param {String} arg2     the name of the database table asociated with this class (if arg1 is the schema name).
 */
function DataCollection(arg1, arg2) {
	const self = this;
	
	const fullTableName = arg2 ? DB.format('??.??', arg1, arg2) : DB.format('??', arg1);
	 ...
	 */
	const getMetadata = DB.executeSql('SHOW COLUMNS FROM ' + fullTableName);
	...
	this.getOne = function(where, whereVal) {
		...
		return self.getMany(where).then(function(results) {
			if (results.length > 1) {throw new Error(`More than 1 record returned for ${fullTableName} where ${where} ${whereVal}.`);}
	...
	this.getMany = function(where) {
		return DB.executeSql('SELECT * FROM ' + fullTableName + DB.where(where)).then(self._asEntity);
	};
	...
	this.getAll = function() {
		return DB.executeSql('SELECT * FROM ' + fullTableName).then(self._asEntity);
	};
	...
	this.count = function(where) {
		const COUNT = 'COUNT(*)';
		const SQL = `SELECT ${COUNT} AS ?? FROM ` + fullTableName + DB.where(where);
		return DB.executeSql(SQL, COUNT).then(self._asEntity);
	};
	...
	this.insert = function(values) {
		...
		return DB.executeSql(`INSERT INTO ${fullTableName} (??) VALUES ?`, columns, values);
	};
	...
	function Entity(rowData) {
		...
		this.insert = function() {
			...
			const SQL = `INSERT INTO ${fullTableName} SET ?`;
			return DB.executeSql(SQL, self._rowData).then(function(result) {
		...
		this.update = function() {
			...
			const SQL = `UPDATE ${fullTableName} SET ?` + idClause();
			return DB.executeSql(SQL, self._rowData).then(result => self);
		};
		
		this.delete = function() {
			return DB.executeSql('DELETE FROM ' + fullTableName + idClause());
		};
